...
def test_g76_hard_pass_limit_raises_semantic_resource_error(monkeypatch):
    monkeypatch.setattr(cycle_module, "checkpoint", lambda _kind: None)

    with pytest.raises(SemanticError, match="G76 exceeds 10000 passes") as exc_info:
        cycle_module._g76_constant_area_depths(  # pylint: disable=protected-access
            total_rad=1.0,
            first_rad=1e-6,
            min_increment_rad=0.0,
            finish_allow_rad=0.0,
