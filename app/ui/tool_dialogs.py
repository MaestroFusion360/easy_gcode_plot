...
from app.ui.generated.milling_tool_editor import Ui_MillingToolEditor
from app.ui.generated.turning_tool_editor import Ui_TurningToolEditor
...
def _accept_unique_tool(editor):
    ...
    if key in editor.reservedCodes:
        QMessageBox.warning(editor, "Duplicate tool number", f"{key} already exists. Choose a different number.")
        return
    editor.accept()
...
class _TurningToolEditor(QDialog):
    ...
    def validateAndAccept(self):
        ...
        if not digits.isdigit() or not 1 <= len(digits) <= 4:
            QMessageBox.warning(self, "Tool Library", "T code must contain 1 to 4 digits.")
            return
        if self.currentToolType() is None:
            QMessageBox.warning(self, "Tool Library", "Select an available tool type.")
            return
...
class _MillingToolEditor(QDialog):
    ...
    def validateAndAccept(self):
        ...
        if not digits.isdigit() or not 1 <= int(digits) <= 99:
            QMessageBox.warning(self, "Tool Library", "T code must be T1-T99.")
            return
        if self.diameter.value() <= 0.0 or self.length.value() <= 0.0:
            QMessageBox.warning(self, "Tool Library", "Diameter and length must be greater than zero.")
            return
        tool_type = self.currentType()
        if tool_type == "mill_bull" and self.cornerRadius.value() > self.diameter.value() / 2.0:
            QMessageBox.warning(self, "Tool Library", "Bull corner radius cannot exceed half the diameter.")
            return
        if tool_type in {"face_mill", "slot_mill"}:
            if not 0.0 < self.cuttingHeight.value() <= self.length.value():
                QMessageBox.warning(self, "Tool Library", "Cutting height must be within the total tool length.")
                return
            if not 0.0 < self.shankDiameter.value() < self.diameter.value():
                QMessageBox.warning(self, "Tool Library", "Shank diameter must be smaller than cutter diameter.")
                return
        if tool_type == "chamfer_mill" and not 0.0 <= self.tipDiameter.value() < self.diameter.value():
            QMessageBox.warning(self, "Tool Library", "Chamfer tip diameter must be smaller than cutter diameter.")
            return
        _accept_unique_tool(self)
