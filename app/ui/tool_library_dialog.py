...
from app.ui.generated.tool_library import Ui_ToolLibraryDialog
from app.ui.tool_dialogs import _export_tool_library, _MillingToolEditor, _TurningToolEditor
...
class ToolLibraryDialog(QDialog):
    ...
    def save_program_to_library(self, kind):
        ...
        editor.setWindowTitle("Save Tool to Library")
    ...
    def duplicate_library_tool(self, kind):
        ...
        if new_key is None:
            QMessageBox.warning(self, "Tool Library", "No free tool number is available.")
            return
    ...
    def _confirm_replace(self, key):
        answer = QMessageBox.question(
            self,
            "Tool Library",
            f"Saved tool {key} already exists. Replace it?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
    ...
    def accept(self):
        if not save_library_changes(self._library_original, self._library_working):
            QMessageBox.warning(self, "Tool Library", "Could not save the tool library. Please retry.")
            return
        for kind in ("milling", "turning"):
