...
def test_stock_dialog_prefills_auto_suggestion_without_saving(qt_app):
    ...
    assert window.stockDlg.front_z.value() == pytest.approx(2.0)
...
def test_stock_dialog_auto_front_z_shows_actual_positive_program_boundary(qt_app):
    window = _lathe_window(qt_app)
    window.turnStockFrontAllowance = 2.0
    window.stockConfigured = False
    window.ui.editor.setText("G21 G18 G90\nG0 X160 Z180\nG1 X140 Z180 F100\nG1 X100 Z100\nG1 X40 Z40\nM30")
    ...
    assert window._stock_auto_suggestion.front_z == pytest.approx(180.0)  # pylint: disable=protected-access
    assert window.stockDlg.front_z.value() == pytest.approx(182.0)
    assert window.stockDlg.ui.frontAllowanceLabel.text() == "Stock front Z"
    window.stockDlg.close()
    window.deleteLater()


def test_stock_dialog_front_z_accepts_negative_program_coordinates(qt_app):
    window = _lathe_window(qt_app)
    window.turnStockFrontAllowance = 0.0
    window.stockConfigured = False
    window.ui.editor.setText("G21 G18 G90\nG0 X40 Z-10\nG1 X40 Z-20 F100\nM30")
    assert window.updateData()
    ...
    assert window.stockDlg.front_z.minimum() < 0.0
    assert window.stockDlg.front_z.value() == pytest.approx(-10.0)
    window.stockDlg.close()
    window.deleteLater()
