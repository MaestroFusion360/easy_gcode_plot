...
from app.gcode.kernel import tool_compensation as compensation
...
def test_tip_orientation_vector_covers_diagonal_axial_and_center_references(orientation, expected):
    assert compensation.tip_orientation_vector(orientation, 0.8) == expected


@pytest.mark.parametrize("orientation", [0, 10, "bad"])
def test_tip_orientation_rejects_values_outside_fanuc_range(orientation):
    with pytest.raises(compensation.ToolCompensationError, match="range 1-9"):
        compensation.tip_orientation_vector(orientation, 0.8)
...
def test_straight_cut_offsets_to_the_correct_side_in_diameter_coordinates(mode, expected_x):
    primitive = compensation._offset_motion(  # pylint: disable=protected-access
...
def test_compensation_joins_both_corner_handednesses_without_a_gap(mode, expected_corner):
    ...
    result = compensation.apply_tool_nose_compensation(motions, {"T0101": TOOL})
...
def test_zero_length_transition_uses_only_the_tip_reference_translation():
    ...
    assert primitive.start == compensation._Vec(12.0, -2.0)  # pylint: disable=protected-access
    assert primitive.end == primitive.start


def test_parallel_disconnected_segments_fail_instead_of_inventing_a_corner():
    first = compensation._offset_motion(  # pylint: disable=protected-access
        _motion((20.0, 0.0), (20.0, -10.0)), 1.0, 9
    )
    second = compensation._offset_motion(  # pylint: disable=protected-access
        _motion((40.0, -10.0), (40.0, -20.0)), 1.0, 9
    )

    with pytest.raises(compensation.ToolCompensationError, match="do not intersect"):
        compensation._join(first, second)  # pylint: disable=protected-access


def test_arc_that_collapses_under_compensation_is_rejected():
    arc = _motion((20.0, 0.0), (0.0, 10.0), move=3, mode=41, i=-20.0, k=0.0)

    with pytest.raises(compensation.ToolCompensationError, match="collapses"):
        compensation._offset_motion(arc, radius=10.0, orientation=9)  # pylint: disable=protected-access


def test_invalid_arc_center_is_rejected_before_offsetting():
    arc = _motion((20.0, 0.0), (0.0, 9.0), move=3, mode=41, i=-20.0, k=0.0)

    with pytest.raises(compensation.ToolCompensationError, match="Invalid tool compensation arc"):
        compensation._offset_motion(arc, radius=1.0, orientation=9)  # pylint: disable=protected-access
...
def test_compensation_exit_starts_at_the_actual_compensated_endpoint():
    ...
    result = compensation.apply_tool_nose_compensation([cutting, exit_move], {"T0101": TOOL})
...
def test_missing_tool_is_reported_and_keeps_the_nominal_trace():
    ...
    assert compensation.missing_compensation_tools([motion], {}) == ("T9999",)
    assert compensation.apply_tool_nose_compensation([motion], {}) == [motion]
...
def test_unsafe_tool_definitions_are_rejected(tool, message):
    with pytest.raises(compensation.ToolCompensationError, match=message):
        compensation.apply_tool_nose_compensation(
