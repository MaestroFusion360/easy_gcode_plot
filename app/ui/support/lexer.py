...
class GcodeLexer(QsciLexerCustom):
    ...
    def initColors(self, colors=None):
        ...
        self.apply_theme(colors or {})

    def apply_theme(self, colors):
        """Apply theme-provided style colors, falling back to the light defaults."""
        paper = QColor(colors.get("paper", "#ffffff"))
        default_text = QColor(colors.get("default_text", "#000000"))
        self.setDefaultPaper(paper)
        self.setDefaultColor(default_text)
        self.setColor(default_text, self.Default)
        self.setColor(QColor(colors.get("rapid", "#ff0000")), self.Rapid)
        self.setColor(QColor(colors.get("linear", "#2ecc71")), self.Linear)
        self.setColor(QColor(colors.get("circular", "#0000ff")), self.Circular)
        for style in self.stylesLexer:
