...
from app.ui.plot.stock_overlay import TurningStockOverlayItem
...
class MainWindowStockMixin:
    ...
    def applyStockSettings(self, values):
        ...
        self.ui.statusbar.showMessage(QCoreApplication.translate("MainWindow", "Turning stock configured"), 5000)
    ...
    def resetStockToAuto(self, *, refresh: bool = True) -> None:
        ...
        if refresh:
            ...
            self._update_stock_outline()
            self.ui.statusbar.showMessage(QCoreApplication.translate("MainWindow", "Turning stock set to Auto"), 5000)
    ...
    def _rebuild_stock_timeline(self) -> bool:
        ...
        if not getattr(self, "stockConfigured", False):
            ...
            self.ui.statusbar.showMessage(
                QCoreApplication.translate("MainWindow", "Configure turning stock in Settings > Stock first."), 10000
            )
            return False
        if result is None or not result.motions:
            self._stock_timeline = None
            self.ui.statusbar.showMessage(
                QCoreApplication.translate("MainWindow", "Stock Removal needs a calculated turning toolpath."), 10000
            )
            return False
        if not result.complete:
            self._stock_timeline = None
            self.ui.statusbar.showMessage(
                QCoreApplication.translate("MainWindow", "Stock Removal needs a complete turning program."), 10000
            )
            return False
        spec = self._effective_stock_spec()
        try:
            ...
            self._stock_timeline = None
            self.ui.statusbar.showMessage(
                QCoreApplication.translate("MainWindow", "Stock Removal is unavailable: {0}").format(exc), 10000
            )
